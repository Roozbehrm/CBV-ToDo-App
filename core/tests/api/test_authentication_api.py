...
class TestTaskApi:
    def test_regiteration(self, client):
        ...
        response = client.post(url, data={'email':'test1@test1.com', 'password':'!@#12345', 'password1':'!@#12345'})
    ...
    def test_user_activation(self, client, user_activation_token, unverified_user):
        url = reverse('accounts:api-v1:activation', kwargs={"token": user_activation_token})
    ...
    def test_profile_view_authorized_unverified_user_forbidden(self, client, unverified_user):
        url = reverse('accounts:api-v1:profile')
    ...
    def test_user_profile_put_data_success(self, client, verified_user):
        url = reverse('accounts:api-v1:profile')
        client.force_login(user=verified_user)
        response = client.put(url, data= {'first_name':'testname', 'last_name':'testlastname' })
    ...
    def test_change_password_success(self, client, verified_user):
        ...
        client.force_login(user= verified_user)
        response = client.put(url, data={'old_password':'!@#12345', 'new_password': '12345!@#', 'new_password1':'12345!@#'})
        assert response.status_code == 200
        
    def test_change_password_wrong_old_password(self, client, verified_user):
        url = reverse('accounts:api-v1:change_password')
        client.force_login(user= verified_user)
        response = client.put(url, data={'old_password':'!@#!@#!@#', 'new_password': '12345!@#', 'new_password1':'12345!@#'})
        assert response.status_code == 400
    ...
    def test_reset_password_verify_valid_token_to_update_password(self, client, verified_user, user_reset_password_token):
        uidb64 = user_reset_password_token['uidb64']
        token = user_reset_password_token['token']
        url = reverse('accounts:api-v1:confirm_reset_password', kwargs={'uidb64':uidb64, 'token': token})
        response = client.get(url)
        assert response.status_code == 202

    
    def test_reset_password_verify_invalid_token_to_update_password(self, client, verified_user, user_reset_password_token):
        uidb64 = user_reset_password_token['uidb64']
        token = user_reset_password_token['token'] + '#'
        url = reverse('accounts:api-v1:confirm_reset_password', kwargs={'uidb64':uidb64, 'token': token})
        response = client.get(url)
    ...
    def test_reset_password_update_password_with_valid_token(self, client, verified_user, user_reset_password_token):
        uidb64 = user_reset_password_token['uidb64']
        token = user_reset_password_token['token']
        url = reverse('accounts:api-v1:confirm_reset_password', kwargs={'uidb64':uidb64, 'token': token})
        response = client.put(url, data={'new_password': '!!@@##12345', 'new_password1':'!!@@##12345'})
        assert response.status_code == 200
        

    def test_reset_password_update_password_with_invalid_token(self, client, verified_user, user_reset_password_token):
        uidb64 = user_reset_password_token['uidb64'] + '#'
        token = user_reset_password_token['token']
        url = reverse('accounts:api-v1:confirm_reset_password', kwargs={'uidb64':uidb64, 'token': token})
        response = client.put(url, data={'new_password': '!!@@##12345', 'new_password1':'!!@@##12345'})
